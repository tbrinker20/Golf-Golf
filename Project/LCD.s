...
wcLoop
		...
		CMP R5, #1 
		BEQ wcLoop 
		;3)clearing PA6 to 0(distingushes btw data vs command in data register bc using same register and both data and command are 8 bits)
		LDR R5, =GPIO_PORTA_DATA_R
		LDR R6, [R5]
		BIC R6, #0x40 ;tells it that command is coming by changing PA6
		STR R6,[R5]
		;Step 4) Writing cmd to DR_R(data reg)
		LDR R5, =SSI0_DR_R 
		STRB R0, [R5] 
		;Step 5,6 (ensuring cmd is fully executed, only way for it to get out of this loop)
wcLoop2
		LDR R4, =SSI0_SR_R 
		LDR R5, [R4]
		AND R5, #0x10
		...
		CMP R5, #1 
		BEQ wcLoop2
		;restoring reg and branching back to main program
		POP{R4-R7}
		BX  LR                          ;   return
...
wdloop
		...
		;Step 4) Writing data to DR_R(data reg)
		LDR R5, =SSI0_DR_R 
		STRB R0, [R5] 
